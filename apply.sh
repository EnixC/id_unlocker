...
original_author="EnixC"
original_name="id_unlocker"
original_urlname="id_unlocker"
original_description="Awesome id_unlocker created by EnixC" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/id_unlocker "${name}"
...
then
    ...
    echo "then run 'id_unlocker' or 'python -m id_unlocker'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'id_unlocker' or 'python -m id_unlocker' or 'poetry run id_unlocker'"
fi
