...
- 🎯 Entry points to execute your program using `python -m <id_unlocker>` or `$ id_unlocker` with basic CLI argument parsing.
...
---
# id_unlocker Flask Application

Awesome id_unlocker created by EnixC
...
```bash
git clone https://github.com/EnixC/id_unlocker id_unlocker
cd id_unlocker
make install
```
...
```bash
id_unlocker create-db   # run once
id_unlocker populate-db  # run once (optional)
id_unlocker add-user -u admin -p 1234  # ads a user
id_unlocker run
```
